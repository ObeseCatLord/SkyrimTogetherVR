...
On Linux, the same Windows build can be driven through a Wine prefix that has Windows PowerShell, xmake, Visual Studio/MSVC or a compatible Windows toolchain, and the required package cache available:
...
The wrapper probes `$PSVersionTable.PSVersion` before running the build script. Wine's built-in `powershell.exe` is only a stub on some installs and can exit successfully without executing the script; this is rejected with a clear error. Install Windows PowerShell or PowerShell 7 in the Wine prefix, or set `STVR_WINE_POWERSHELL` to the real executable.
