...
- Added explicit experimental VRIK/HIGGS remote-avatar validation targets `SkyrimTogetherVRClientAvatarSync` and `SkyrimVRImmersiveLauncherAvatarSync`, which build `SkyrimTogetherVRAvatarSync.exe` in staged connection-only mode. The current targets use `VRAvatarService` plus the CommonLib gameplay bridge for same-cell actor lifecycle/root movement; HMD/hand/VRIK data remains observation-only.
...
- Added VR pose snapshot capture:
  ...
  - `SkyrimTogetherVRClientAvatarSync` builds with `TP_SKYRIM_VR_ENABLE_CONNECTION_ONLY=1` and `TP_SKYRIM_VR_ENABLE_REMOTE_AVATAR_SYNC=1`; CommonLib owns same-cell actor lifecycle/root movement, while skeleton, equipment, HIGGS, PLANCK, and FBT mutations remain disabled
  ...
  - the former `CharacterService` avatar handoff schema is legacy; bridge-native avatar diagnostics must replace it before the strict avatar runtime audit is treated as acceptance evidence for the new ownership model
