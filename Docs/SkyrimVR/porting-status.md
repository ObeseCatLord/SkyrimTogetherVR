...
- Added explicit experimental VRIK/HIGGS remote-avatar validation targets `SkyrimTogetherVRClientAvatarSync` and `SkyrimVRImmersiveLauncherAvatarSync`, which build `SkyrimTogetherVRAvatarSync.exe` in staged connection-only mode with only `CharacterService` and the guarded remote HMD/hand/VRIK actor-node consumer enabled.
...
- Added `Tools/SkyrimVR/audit_remote_player_proxy.py` to lock down the `SkyrimTogetherVR.remoteplayers` writer/parser/doc schema, run a temp-directory parser fixture through `vr_handoff.build_readout_payload()`, expose separate VRIK avatar and HIGGS-aware avatar readiness fields, and keep the default remote-player proxy readout free of actor/object/equipment/HIGGS/PLANCK mutation calls.
...
- Added `Tools/SkyrimVR/audit_bringup_hooks.py` for repeatable validation that the default VR client keeps dangerous hooks/inline patches disabled while retaining the first-run startup, main-loop, VM-update, render-init, and pose-node log breadcrumbs.
...
- Added VR connection-only mode:
  ...
  - ticks reduced `World::Update()` from the VR VM update hook
  ...
  - adds `SkyrimTogetherUtils` Papyrus natives for VR scripts to connect, disconnect, query online state, save the remembered endpoint/password to `SkyrimTogetherVR.connection`, read the current connection state string, and read the remembered configured endpoint/password from `SkyrimTogetherVR.connection`
  - adds compiled `SkyrimTogetherVRConnectionMenu.psc/.pex` helpers for endpoint configuration, configure-and-connect, connect, disconnect, configured toggle, localhost toggle, status, and telemetry message-box control
  ...
  - grants the VR package spell form `0x1825` as `Skyrim Together VR` from `SkyrimTogetherVerifyLaunchScript` on quest init and player load
  - patches the existing VR ESP effect form `0x1824` with `SkyrimTogetherVRConnectionSpellEffect`, which calls `SkyrimTogetherVRConnectionMenu.ToggleConfigured()`
  - adds `Tools/SkyrimVR/patch_vr_menu_esp.py` to reapply or check the staged VR connection spell/effect patch
  - installs the Papyrus native-binding hook from the VR bring-up hook set without enabling the broader unvalidated gameplay hook batch, and logs Skyrim Together native bind failures from the CommonLib-shaped `BindNativeMethod` return value
  - documents the mode in `Docs/SkyrimVR/connection-only-mode.md`
- Added a VR overlay/input boundary gate:
  - the default VR target uses the companion panel and Papyrus message-box control surfaces instead of the flat CEF/D3D11 overlay
...
- Added VR pose snapshot capture:
  ...
  - `VRPoseService` ticks in connection-only mode, logs local pose snapshots, sends `RequestVRPoseUpdate` at 20 Hz when connected, and stores remote `NotifyVRPoseUpdate` payloads
  ...
  - the default connection-only target now constructs `VRRemotePlayerService`, which writes `Data/SkyrimTogetherReborn/SkyrimTogetherVR.remoteplayers` by joining server player join/cell/leave notifications with remote pose, VRIK, movement, equipment, activation, magic, combat, projectile, grab, and HIGGS relay maps; this gives the companion `proxy` column and Papyrus telemetry a stable remote-player row with matched stream counts, same-cell/same-worldspace hints, action-lane availability, explicit HMD/left-hand/right-hand target availability, explicit VRIK avatar-validation readiness, and separate HIGGS-aware avatar-validation readiness without spawning actors, creating marker objects, moving scene nodes, equipping items, replaying gameplay actions, or replaying HIGGS/PLANCK state
  - the staged avatar-sync build constructs `VRPoseService` before `CharacterService`, and `CharacterService` stages remote player IK targets on `RemoteVRPoseComponent` by matching pose player ids to remote actor `PlayerComponent` ids
  - remote avatar pose/equipment components are now service-owned cache copies: `VRPoseService` and `VRInventoryService` own remote player liveness, and `CharacterService` removes avatar cache components immediately once the corresponding service map no longer has that player id
  - `TP_SKYRIM_VR_ENABLE_REMOTE_AVATAR_ACTOR_TARGETS=0` keeps remote avatar actor movement/application disabled in the default `SkyrimTogetherVRClient` target until the VR actor and IK call sites are validated
  - `SkyrimTogetherVRClientAvatarSync` builds with `TP_SKYRIM_VR_ENABLE_CONNECTION_ONLY=1`, `TP_SKYRIM_VR_ENABLE_REMOTE_AVATAR_SYNC=1`, and `TP_SKYRIM_VR_ENABLE_REMOTE_AVATAR_ACTOR_TARGETS=1`; actor skeleton writes are separately gated by `TP_SKYRIM_VR_ENABLE_REMOTE_AVATAR_SKELETON_WRITES=0`, so that target now validates same-space actor targeting, remote pose/equipment/VRIK cache joins, and VRIK payload quality without writing directly into the remote actor skeleton
  ...
  - `SkyrimTogetherVRGameplayClient` and `SkyrimVRImmersiveLauncherGameplay` build `SkyrimTogetherVRGameplay.exe` with connection-only mode off, normal gameplay services enabled, VR relay/remote-avatar services enabled, unvalidated flat-Skyrim hooks disabled, and the flat D3D overlay disabled by default
  - the guarded avatar consumer now combines remote `VRMovementService` snapshots with remote pose: a same-space guard requires local/remote movement data and matching cell/worldspace before actor-target cache validation proceeds; direct actor root movement authority and HMD/hand skeleton writes are disabled until a ghost/overlay avatar renderer or a validated animation integration replaces the unstable `NiAVObject::world` write path
  - `CharacterService` writes `Data/SkyrimTogetherReborn/SkyrimTogetherVR.avatar` in the explicit avatar-sync branch with remote pose match counts, actor target attempts, skeleton-write/movement-authority policy flags, VRIK API/finger/camera payload counts, movement/fallback movement counts, rejected invalid transform/VRIK/movement counts, spell origin/destination, arrow origin/destination, bow aim/rotation, weapon-offset, magic-offset/aim validity counts, remote equipment match/upsert/stale counts, queued weapon-draw visual counts, latest pose/VRIK-lane booleans, latest equipment sequence/drawn flags, and the latest player/form/pose sequence result so two-client tests can verify that remote actor targets are being matched with combat-intent, held-item, and VRIK context without mutating their skeletons
  - the avatar handoff now reports same-space guard fields including `last.sameSpaceForApply`, `actorTargetSkippedDifferentCellCount`, `actorTargetSkippedDifferentWorldSpaceCount`, and missing local/remote movement counts so two-client tests can distinguish pose relay success from intentionally skipped actor application
...
- `SkyrimTogetherPlayerAliasScript.pex` grants the connection spell on player load.
...
- Final source-side compile check on this Linux host passes for `SkyrimEncoding`, `CommonLib`, `TPTests`, and `SkyrimTogetherServer`; the actual Windows VR client/launcher build remains blocked before compilation because the current Wine prefix exposes only Wine's built-in `powershell.exe` stub.
...
- `Tools/SkyrimVR/audit_bringup_hooks.py` passes and confirms the default VR target keeps dangerous hooks, inline patches, and remote avatar actor targets disabled while preserving first-run startup, main-loop, VM-update, render-init, pose-node log breadcrumbs, and the explicit avatar-sync-only target configuration.
...
- `Tools/SkyrimVR/audit_remote_player_proxy.py` passes and confirms the default `SkyrimTogetherVR.remoteplayers` file contract matches the writer, companion parser, Papyrus readout, and docs while keeping the proxy service readout-only.
...
- Run the full gameplay package from `artifacts\SkyrimTogetherVR\packages\gameplay`, then collect strict gameplay runtime evidence with `CollectSkyrimTogetherVRGameplayEvidence-Windows.bat`.
...
- Validate VR player cell/grid/level request flow against a server in the default, avatar-sync, and gameplay packages so the normal gameplay package's `PlayerService` path is covered by the same evidence.
...
- Runtime-validate `SkyrimTogetherVRAvatarSync.exe` with two clients, VRIK installed, and HIGGS installed, then replace the provisional direct scene-node writes with a proper VRIK/animation integration if needed.
- Runtime-validate the default `SkyrimTogetherVR.remoteplayers` proxy readout with two clients, then extend it to a stable in-game remote-player debug marker or proxy avatar path that can remain enabled by default.
- Validate the packaged `Skyrim Together VR` spell/power entry point in-game.
