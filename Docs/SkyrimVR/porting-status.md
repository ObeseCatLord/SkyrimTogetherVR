...
- Added VR pose snapshot capture:
  ...
  - `VRAvatarService` consumes canonical character spawn/remove and reference-movement messages, rejects actors outside the local cell/worldspace, and submits only bounded lifecycle/root commands to the CommonLib bridge; direct HMD/hand/VRIK skeleton writes remain disabled
  - the former `CharacterService` avatar handoff schema is legacy; `VRAvatarService` now writes the versioned `commonlib_bridge_v1` schema, and the strict avatar runtime audit requires an active lifecycle epoch, a successful remote actor create, an accepted canonical remote movement packet, at least one submitted root update, and zero rejected commands or invalid transforms
