...
| Add Windows crash-policy coverage. | Adapted | `TPTests` compiles the real `CrashHandler.cpp`. Tests exercise a real frame-handled AV plus outermost installation, all prior dispositions, guard latching/rearming, recursive entry, context extraction, and unreadable RSP. Terminal process behavior remains a runtime gate because deliberately terminating the shared Catch2 process would reduce diagnostic precision. |
