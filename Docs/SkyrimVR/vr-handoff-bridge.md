...
The panel defaults to `http://127.0.0.1:8765/`, shows compact connection and telemetry readouts, and writes the same connect/disconnect command file as the CLI commands. In avatar-sync/gameplay packages it also shows a `Remote Players` table built from the optional pose/avatar/proxy and gameplay relay files. The compatibility panel reports which lanes are disabled or enabled. The PLANCK panel reads `SkyrimTogetherVR.planck` and keeps its nontrivial hit-data probe disabled. The explicit broader-target `proxy` column comes from `SkyrimTogetherVR.remoteplayers` and joins server identity/cell state with replicated relay caches without creating actors or objects. It reports `avatarReady=yes` and `higgsAvatarReady=yes` only when their prerequisites are present. Connect actions update the remembered endpoint in `SkyrimTogetherVR.connection`. The panel accepts `--no-open-browser` for background-only use.

Broader-target pose summaries retain compact diagnostics such as
`spell=yes/yes` and `weapon=yes/no`; these fields are absent from the narrow
default connection-proof target.
...
The gameplay handoff wrapper proves local canonical readiness only. It cannot promote local captured/sent/applied counters to two-client proof; use the paired evidence-zip audit for that claim. Explicit avatar-sync or HIGGS/grab flags remain the separate VR-extension profiles.

The runtime evidence collector is the handoff bundle step for failures or remote review. `Tools/SkyrimVR/collect_runtime_evidence.py` and the packaged evidence collector wrapper `CollectSkyrimTogetherVREvidence-Windows.bat` create a zip with `SkyrimTogetherVR_BuildManifest.json`, `logs\tp_client.log` when available, the handoff readouts/control files, discovered SKSEVR logs, `runtime_audit.txt`, `runtime_checklist.json`, `runtime_checklist.txt`, and `manifest.json`. The checklist maps observed handoff data back to the port's runtime lanes, including package build manifest, startup logging, connection, HMD/hand pose, VRIK, remote avatar readiness, HIGGS-aware remote avatar readiness, discovery schema, player cell status, movement, equipment, activation, magic, combat, projectile, grab, HIGGS/PLANCK compatibility policy, HIGGS relay, PLANCK bridge status, and save/load. Use `--gameplay-bootstrap` while one gameplay-package client is still running to require gameplay package identity plus live connection, lifecycle, current cell, local avatar, local pose, VRIK, and HIGGS evidence without claiming remote-player or relay coverage. Live admission always requires the status, lifecycle, player-cell, avatar, and aggregate gameplay readouts to carry one fresh launch nonce and process identity. The gameplay snapshot must always be structurally valid; `ready=1` is required only for gameplay and gameplay-bootstrap profiles. The status must report equal nonempty client/server network versions, protocol revision 21, and nonzero server/session/generation identities; the matching native bridge mapping validates capability revision 34 and mapping ABI 24. Baseline crash evidence may retain stale or incomplete files, but is marked untrusted and cannot satisfy a live acceptance gate. `--gameplay` collects one trusted side of a two-client proof. Audit it with `--require-gameplay --peer-archive PEER.zip`: both archives must be trusted live gameplay evidence from distinct players in one session, and bidirectional counters must cover every mandatory canonical domain. Direct HIGGS/grab proof remains separately requested; PLANCK physical events are now the optional `PlanckPhysicsInterface002` relay, advertised only after the local interface002 feature set is operational and negotiated by both endpoints, never as part of the base gameplay profile. The package build manifest check verifies Windows x64 package schema, expected default/avatar-sync/gameplay targets, copied artifacts, staged game files, companion helper state, exact network version, clean source revision, and source provenance. The discovery check validates grid fields, cached cell/worldspace ids, actor count/limit, and capped `actor.N.formId` rows structurally rather than expecting exact actor IDs. The player cell check validates readiness, request counters, last grid/cell ids, grid coords, and level-send state from `SkyrimTogetherVR.playercell`. The collector does not launch Skyrim or mutate the game install; source-tree runs default to `artifacts/SkyrimTogetherVR/runtime-evidence`, and packaged runs default to `Documents/SkyrimTogetherVRRuntimeEvidence`.
...
The evidence zip audit is the offline review step. `Tools/SkyrimVR/audit_runtime_evidence_zip.py` and packaged evidence zip audit wrapper `AuditSkyrimTogetherVREvidence-Windows.bat` read a collected zip without needing access to the game install, check `manifest.json`, `package/SkyrimTogetherVR_BuildManifest.json`, `runtime_audit.txt`, `runtime_checklist.json`, `runtime_checklist.txt`, the scope-required log/handoff entries, and fail on missing required files, invalid package build manifest, nonzero embedded runtime audit, or failed required checklist lanes. Use `--require-gameplay-bootstrap` for the one-client gameplay-package acceptance profile, `--require-remote-player` for a default two-client remote-player proxy run, `--require-avatar-sync` for mandatory VRIK/HIGGS remote-avatar validation, and `--require-gameplay` for a full gameplay package run. Add `--require-vr-pose-context` or a matching `--require-*-relay` flag only when that lane was deliberately exercised.
