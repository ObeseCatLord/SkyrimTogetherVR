...
void VRMovementRelayService::OnVRMovementUpdate(const PacketEvent<RequestVRMovementUpdate>& acMessage) noexcept try
{
    ...
    if (!SkyrimTogether::Protocol::HasCapability(
            acMessage.pPlayer->GetGameplayCapabilities(), SkyrimTogether::Protocol::GameplayCapability::VRMovementRelay))
        return;
    ...
    if (!character || !GameServer::Get()->SendToPlayersWithCapabilitiesInRange(
            notify, *character,
            SkyrimTogether::Protocol::ToMask(SkyrimTogether::Protocol::GameplayCapability::VRMovementRelay),
            acMessage.pPlayer))
    {
        if (VRRelayLogPolicy::RecordNoRoutableCharacter(m_noRoutableCharacterCount))
            spdlog::warn("VR movement relay dropped because sender has no routable character (aggregate count: {})",
