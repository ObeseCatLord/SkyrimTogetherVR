...
bool VRAvatarService::CanSubmitAvatarCommands() noexcept
{
    ...
    const bool negotiated = SkyrimTogether::Protocol::HasCapability(networkCapabilities, SkyrimTogether::Protocol::GameplayCapability::RemoteAvatarLifecycle) &&
                            SkyrimTogether::Protocol::HasCapability(networkCapabilities, SkyrimTogether::Protocol::GameplayCapability::RemoteRootTransform);
    const bool bridgeActive = SkyrimTogetherVR::GameplayBridgeClient::IsReady() &&
    ...
    if (negotiated && bridgeActive)
        return true;

    if (!m_capabilityWarningLogged)
    {
        m_capabilityWarningLogged = true;
        spdlog::warn("VR avatar service is inert until negotiated remote-avatar and active bridge capabilities are available");
    }
    return false;
}
